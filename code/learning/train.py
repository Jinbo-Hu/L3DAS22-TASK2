    ...
    for batch_sample in iterator:
        ...
        if it % int(1*batchNum_per_epoch) == 0 :
            ...
            for k, v in train_losses.items():
                train_losses[k] = reduce_value(v) / batchNum_per_epoch
            if cfg['training']['valid_set']:
        ...
        if rank == 0:
            if rem_batch == 0 and it > 0:
                if cfg['training']['valid_set']:
                    ckptIO.save(epoch_it, it, metrics=valid_metrics, key_rank='F score', rank_order='high', max_epoch=max_epoch)
                else:
